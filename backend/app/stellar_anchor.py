...
from .models import AgentLog, TransactionRecord
...
class StellarAnchorService:
    ...
    def discover_toml(self) -> dict[str, Any]:
        ...
        if not web_auth or not transfer:
            raise RuntimeError("stellar.toml içinde WEB_AUTH_ENDPOINT / TRANSFER_SERVER yok")
        self._toml = parsed
        return parsed
    ...
    async def sep10_authenticate(self, kp: Optional[Keypair] = None) -> str:
        ...
        if not token:
            raise RuntimeError("SEP-10 JWT alınamadı")
        return token
    ...
    def _buy_usdc_on_dex(self, kp: Keypair, usdc: Asset, dest_amount: float) -> None:
        """Fill USDC immediately via path payment (manage_buy_offer can rest unfilled)."""
        ...
        if not records:
            raise RuntimeError(f"Testnet DEX'te {dest_str} USDC için path yok")
        best = records[0]
